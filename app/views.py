...
def check_image(request):
    if request.method == 'POST':
        ...
        import base64
        import random
        # print(face_img_data.split(',')[-1])
        face_image_name = f"image_{random.randint(1, 1000)}_face_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
        base_image_name = f"image_{random.randint(1001, 1500)}_base_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
        face_decodeit = open(f'images/{face_image_name}.jpg', 'wb')
        face_decodeit.write(base64.b64decode(face_img_data.split(',')[-1]))
        face_decodeit.close()
        base_decodeit = open(f'images/{base_image_name}.jpg', 'wb')
